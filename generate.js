...
const fileLines = filesWithMetaData.map(
  ({ name, file, importName }) =>
    `import ${importName} from '${path.relative(
      path.join(exportPath),
      path.join(importPath, file)
    )}'`
);
fileLines.push("");

fileLines.push(
  "export const techIcons: Array<{src: string; name: string}> = ["
);
for (const fileWithMetaData of filesWithMetaData) {
  fileLines.push(
    `  { name: "${fileWithMetaData.name}", src: ${fileWithMetaData.importName} },`
  );
}
fileLines.push(`];`);

fs.writeFileSync(
  path.join(exportPath, "imgTechIcons.ts"),
  fileLines.join("\n")
);
